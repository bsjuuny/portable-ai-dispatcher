...
import {
  assembleMacOSPortableKit,
  resolvePortableTarget,
  type PortableKitBuildResult,
  type PortableTarget,
} from './portable-kit-macos.js';

export type PortableKitResult = PortableKitBuildResult;
export type { PortableTarget } from './portable-kit-macos.js';

function portableConfig(): string {
  const configured = PORTABLE_CONFIG.replace(
    'safety:\n  autoApply:\n    enabled: true\n',
    'safety:\n  autoApply:\n    enabled: true\n    requireIndependentReview: true\n',
  );
  if (configured === PORTABLE_CONFIG) throw new Error('Portable config is missing the expected auto-apply policy block.');
  return configured;
}

function portableReadme(): string {
  return PORTABLE_README.replace(
    ...
    'This kit enables safety.autoApply and requireIndependentReview. A change reaches your real files only after validation and review by a genuinely separate provider. With only one ready local model, self-review still produces diagnostics but the change is withheld as BLOCKED_BY_POLICY. Validation/review failures and high-risk changes are also withheld. Commit before running a task if you want an easy way back.',
  );
}

function portablePrepGuide(): string {
  return PORTABLE_KIT_PREP_GUIDE
    .replace(
      '  2. bin\\start-cpu16.cmd(16GB PC) 또는 bin\\start-cpu32.cmd(32GB PC) 실행',
      '  2. bin\\start-cpu16.cmd 또는 bin\\start-cpu32.cmd 실행 (preflight launch plan 자동 사용)',
    )
    .replace(
      '     (또는 -32). 검증+리뷰까지 자동으로 통과하면 사람 승인 없이 바로 실제 파일에\n     반영됩니다.',
      '     (또는 -32). 검증과 실제로 분리된 reviewer의 리뷰까지 통과해야\n     실제 파일에 반영됩니다. 자가 리뷰만 가능하면 BLOCKED_BY_POLICY로 보류됩니다.',
    )
    .replace(
      '  - auto-apply 기본 켜짐: 검증(빌드/린트/테스트)과 독립된 다른 로컬 모델의\n    리뷰까지 통과한 변경은 사람 승인 없이 바로 반영됩니다. 위험도가 높다고\n    판단되거나 검증/리뷰에서 걸리면 반영되지 않고 버려집니다.',
      '  - auto-apply와 독립 리뷰 필수 정책이 기본으로 켜져 있습니다. 하나의 로컬 모델이\n    자가 리뷰한 변경은 자동 반영되지 않고 BLOCKED_BY_POLICY로 보류됩니다.\n    무인 반영은 구현 모델과 실제로 분리된 reviewer provider를 구성한 후에만 사용하세요.',
    );
}

/** Creates a self-contained USB layout from a built Dispatcher checkout. */
export function assemblePortableKit(
  sourceRoot: string,
  destinationDirectory: string,
  includeAssets = true,
  nodeExecutable = process.execPath,
  requestedTarget?: PortableTarget | string,
): PortableKitResult {
  const target = resolvePortableTarget(requestedTarget);
  if (target === 'macos-arm64') {
    return assembleMacOSPortableKit(sourceRoot, destinationDirectory, includeAssets, portableConfig());
  }
  ...
  writeFileSync(join(destination, 'bin', 'new-react-project.cmd'), NEW_REACT_PROJECT, 'utf8');
  writeFileSync(join(destination, 'bin', 'new-vue-project.cmd'), NEW_VUE_PROJECT, 'utf8');
  writeFileSync(join(destination, 'README.txt'), portableReadme(), 'utf8');
  writeFileSync(join(destination, '사용방법.txt'), PORTABLE_KOREAN_GUIDE, 'utf8');
  writeFileSync(join(destination, 'kit-준비-가이드.txt'), portablePrepGuide(), 'utf8');
  writeFileSync(join(destination, 'portable-kit.json'), `${JSON.stringify({ schemaVersion: '1', kitFolderName: 'portable-ai-dispatcher', target: 'windows-x64', os: 'windows', arch: 'x64' }, null, 2)}\n`, 'utf8');
  ...
  writeFileSync(join(destination, 'templates', 'PUT_PROJECT_TEMPLATES_HERE.txt'), 'Place ready-to-use project templates here (nextjs-starter/, react-starter/, vue-starter/), each already a git repo with dependencies installed (node_modules committed to disk, gitignored from tracking) so bin\\new-*-project.cmd can copy one with zero network access. Scaffolding an empty repo from scratch through the local coding agent is slow and unreliable at this model size (live-reproduced 2026-08-23); a pre-built, already-working template that the AI only adds features to is far more reliable offline.\n', 'utf8');
  ...
  const hasReactTemplate = existsSync(join(destination, 'templates', 'react-starter', 'node_modules'));
  const hasVueTemplate = existsSync(join(destination, 'templates', 'vue-starter', 'node_modules'));
  ...
  return {
    ...
    requiredBeforeUse: [
      ...
      // Optional, but bin\new-*-project.cmd needs the matching one - not required
      // for the rest of the kit to work, only for that one convenience script.
      ...(hasNextjsTemplate ? [] : ['(optional) a pre-built templates/nextjs-starter/ with node_modules installed, for bin\\new-nextjs-project.cmd']),
      ...(hasReactTemplate ? [] : ['(optional) a pre-built templates/react-starter/ with node_modules installed, for bin\\new-react-project.cmd']),
      ...(hasVueTemplate ? [] : ['(optional) a pre-built templates/vue-starter/ with node_modules installed, for bin\\new-vue-project.cmd']),
    ],
  };
}
...
export function copyPortableNodeRuntime(destinationDirectory: string, nodeExecutable = process.execPath): void {
  const targetKind = readPortableKitTarget(destinationDirectory);
  if (targetKind === 'macos-arm64') {
    throw new Error('macos-arm64 kits require a complete Node.js distribution under runtime/node; copying a single host executable is not portable.');
  }
  ...
  const target = join(targetDirectory, 'node.exe');
...
function readPortableKitTarget(destinationDirectory: string): PortableTarget {
  const manifestPath = join(resolve(destinationDirectory), 'portable-kit.json');
  if (!existsSync(manifestPath)) return resolvePortableTarget();
  try {
    const manifest = JSON.parse(readFileSync(manifestPath, 'utf8')) as { target?: string };
    return resolvePortableTarget(manifest.target);
  } catch (cause) {
    throw new Error(`Invalid portable-kit.json: ${(cause as Error).message}`, { cause });
  }
}
...
const CPU16_STARTER = `@echo off\r\ncall "%~dp0check-vcredist.cmd" || exit /b 1\r\ncall "%~dp0ai-dispatcher.cmd" local start %*\r\nexit /b %ERRORLEVEL%\r\n`;

const CPU32_STARTER = `@echo off\r\ncall "%~dp0check-vcredist.cmd" || exit /b 1\r\ncall "%~dp0ai-dispatcher.cmd" local start %*\r\nexit /b %ERRORLEVEL%\r\n`;

/** Every bin\new-*-project.cmd is the same copy-a-prebuilt-template script, just
 * pointed at a different templates\ subfolder - see PUT_PROJECT_TEMPLATES_HERE.txt
 * for why a pre-built template beats asking the local model to scaffold from empty. */
function newProjectScript(scriptName: string, templateFolder: string, displayName: string): string {
  return [
    '@echo off',
    'setlocal EnableExtensions',
    'set "KIT_ROOT=%~dp0.."',
    `set "TEMPLATE=%KIT_ROOT%\\templates\\${templateFolder}"`,
    'if "%~1"=="" (',
    `  echo Usage: ${scriptName} DESTINATION_FOLDER`,
    '  exit /b 1',
    ')',
    'set "DEST=%~1"',
    'if exist "%DEST%" (',
    ...
    `  echo Bundled ${displayName} template is missing or incomplete: %TEMPLATE%`,
    `  echo Put a pre-built template ^(node_modules installed^) in templates\\${templateFolder} and try again.`,
    '  exit /b 1',
    ')',
    `echo Copying ${displayName} starter to %DEST% ^(this includes node_modules, so it takes a bit^)...`,
...
const NEW_NEXTJS_PROJECT = newProjectScript('new-nextjs-project.cmd', 'nextjs-starter', 'Next.js');
const NEW_REACT_PROJECT = newProjectScript('new-react-project.cmd', 'react-starter', 'React');
const NEW_VUE_PROJECT = newProjectScript('new-vue-project.cmd', 'vue-starter', 'Vue');
...
1. 준비 상태 확인
   ...
   - RAM과 실행 환경에 맞는 runtime·모델·context는 preflight가 하나의 launch plan으로 선택합니다.
...
2. 모델 서버 시작
   - bin\\start-cpu16.cmd 또는 bin\\start-cpu32.cmd를 실행합니다. 두 런처 모두 같은
     preflight launch plan을 사용하므로 메모리 기준을 수동으로 중복 판단하지 않습니다.
   - 창을 닫지 말고 유지합니다. 서버는 이 PC 안에서만(127.0.0.1:8080) 동작합니다.
   - 메모리가 부족하면 프로그램을 닫고 16GB용 모델을 사용합니다.

3. 새 프로젝트 시작하기 (Next.js / React / Vue)
   ...
   - Next.js: X:\\portable-ai-dispatcher\\bin\\new-nextjs-project.cmd C:\\작업할\\새\\프로젝트
   - React: X:\\portable-ai-dispatcher\\bin\\new-react-project.cmd C:\\작업할\\새\\프로젝트
   - Vue: X:\\portable-ai-dispatcher\\bin\\new-vue-project.cmd C:\\작업할\\새\\프로젝트
...
4. AI Dispatcher 실행
   ...
   - 자체 검증(빌드/테스트)과 구현 모델과 분리된 reviewer의 리뷰까지 통과해야
     사람 승인 없이 실제 파일에 반영(auto-apply)됩니다. 하나의 로컬 모델만 있어
     자가 리뷰한 경우에는 진단 결과는 남지만 변경은 BLOCKED_BY_POLICY로 보류됩니다.
     무인 자동 반영이 필요하면 실제로 별도인 reviewer provider를 구성하세요.
...
6. git
   ...
   - 다만 실제로 작업할 프로젝트 폴더는 그 폴더 자체가 이미 git 저장소여야 합니다
     ...
     (3번의 new-nextjs/react/vue-project.cmd로 만든 폴더는 이미 git 저장소라 이 단계가 필요 없습니다.)
...
8. 라이선스와 보안
   ...
   - templates\\nextjs-starter, templates\\react-starter, templates\\vue-starter는 각각 공식 스캐폴딩
     도구(create-next-app, create-vite)로 만든 결과물이며 MIT 라이선스입니다.
