...
2. **`bin\start-cpu16.cmd`**(16GB PC) 또는 **`bin\start-cpu32.cmd`**(32GB PC) 실행 → 로딩 로그가 끝까지 나올 때까지 창을 유지. 시작하자마자 아무 로그도 없이 바로 프롬프트로 돌아오면 → 3번 문제해결 참고.
...
4. **작업 요청**: `bin\ai-dispatcher.cmd implement "요청 내용" --cwd "그 폴더" --provider local-cpu-16` (또는 `-32`). 검증+리뷰까지 자동으로 통과하면 사람 승인 없이 바로 실제 파일에 반영됩니다.
...
- **auto-apply 기본 켜짐**: 검증(빌드/린트/테스트)과 독립된 다른 로컬 모델의 리뷰까지 통과한 변경은 사람 승인 없이 바로 반영됩니다. 위험도가 높다고 판단되거나 검증/리뷰에서 걸리면 반영되지 않고 버려집니다.
